...
3. **OBJ-frontend-engineer-4** | Distinguish "layer failed to fetch" from "layer legitimately has 0 features" in the panel (`docs/app.js` `loadAllData`/`renderLayerList`)
   unlocks:  a regulator or DC developer reading a layer count of "0" in the sidebar can tell whether that infrastructure category genuinely has no data or the file simply failed to load, instead of the two being visually identical
   evidence: `loadAllData`'s catch block (`docs/app.js:248-258`) silently sets a failed layer's data to an empty FeatureCollection and only `console.warn`s; `renderLayerList` (`app.js:289`) then renders that layer's count as plain "0", indistinguishable from a real zero. A deliberately broken file path must instead surface a visible failure signal, not a normal-looking "0" row
   size:     S
   risk:     low — distinct from OBJ-map-debugger-3 (MapLibre's own post-load `error` event, not `app.js`'s `fetch()` path) and additive to OBJ-frontend-engineer-2 (loading state, not failure state); touches only the existing catch block + row markup
...
4. **OBJ-map-tester-4** | Document that this sandbox's egress policy blocks `unpkg.com` (confirmed: `curl https://unpkg.com/... → CONNECT tunnel failed, response 403`), so no sitting run from this environment can produce genuine popup-click/MapLibre-runtime browser evidence
   unlocks:  the next seat citing "browser-level evidence" for a change that touches map-runtime behavior (not just static HTML) knows up front that this sandbox cannot independently produce it, and must either rely on the executing seat's evidence and say so explicitly, or escalate for a network exception — instead of each seat separately rediscovering the same 403 mid-verification
   evidence: a one-line note (in BOARD.md or a CLAUDE.md-adjacent doc) stating the blocked host and its implication for future GO/GO-STATIC calls
   size:     S
   risk:     none — documentation only, doesn't change any check
...
| 2026-08-18 | 22:53 | OBJ-map-debugger-1 | *(pending — this sitting's PR)* | a regulator or DC developer who spots a wrong coordinate or stale figure can actually get the correction to a real, monitored mailbox instead of an undeliverable `example.com` placeholder |
