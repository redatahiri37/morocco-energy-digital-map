...
1. **OBJ-map-debugger-5** | Give `interconnectors.geojson` and `planned-corridors.geojson` distinct MapLibre source ids in `buildLineLayer()` (`docs/app.js:489-494,527-528`)
   ...
   chair note: re-confirmed live on `main` (`docs/app.js:528`, `const srcId = "src-grid"`, hardcoded and shared by both call sites) at this sitting, 2026-08-07 08:01 — top-ranked candidate for the next open SHIP slot (budget stays closed until ≈2026-08-07 22:21 UTC / 00:21 Paris, 24h after the last ship commit `7c27e5a`)
