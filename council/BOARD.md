...
### coord-validator
1. **OBJ-coord-validator-4** | Correct or withdraw the WBG-2018 citation on `docs/data/morocco/transmission-lines.geojson` (new, minted 2026-08-19 23:15, from the OBJ-coord-validator-2 investigation below)
...
3. **OBJ-coord-validator-2** | Sample-verify `docs/data/morocco/national-hv.geojson` (947 ONEE 60 kV line features, `coord_method: osm_derived`) and `docs/data/morocco/transmission-lines.geojson` (541 WBG line features) — **investigated 2026-08-19 23:15, partially resolved, kept open on the live-Nominatim gap**
   ...
   evidence: **2026-08-19 findings** — Method 1 (bbox/degenerate-geometry, full population 1,488/1,488): PASS, 0 vertices outside Morocco, 0 degenerate geometries. Method 2 (live Nominatim confirmation, the originally-specified method): BLOCKED, 0/1,488 — `nominatim.openstreetmap.org` returned a 403 policy denial from this session's egress proxy; per the proxy's own guidance this was not retried or routed around. Method 3 (geographic-plausibility fallback vs. ~34 known-hub coordinates, full population 2,976/2,976 endpoints): WARN, 82–92% within 50–75km of a known hub, two outliers individually confirmed as real corridors (Figuig, Boujdour–Dakhla), not authoritative. Method 4 (cross-file duplication/provenance, full population 1,488/1,488): **FAIL — see new OBJ-coord-validator-4**, spun out as its own objective since it's now an actionable, evidenced citation problem rather than an open question. Routing/geometry itself is provisionally trusted (Methods 1+3); the live-Nominatim leg (Method 2) stays the open gap — re-run once network egress to Nominatim is available.
   size:     M — Method 2 (the only leg still needed for full confirmation) requires working Nominatim access, which this session does not have; not re-attemptable until that changes
...
2. **OBJ-map-tester-2** | Write down what "browser-level evidence" must contain for a release-gate GO (desktop/light/375px console + screenshot requirements)
   ...
   note (2026-08-19): this sitting hit the gap directly — this sandbox's own egress policy blocks unpkg.com, so MapLibre can't construct from the real CDN URL in headless testing here. Workaround established and reproduced twice: `page.route()` intercept the two `unpkg.com/maplibre-gl@4.7.1/...` requests to a local `npm install maplibre-gl@4.7.1` copy (test-harness only, `docs/index.html` is untouched and still points at unpkg for the real deploy), then use `map.queryRenderedFeatures()` to find a real feature's screen pixel instead of guessing coordinates. Worth folding into the written checklist as the standard technique.
...
### platform-engineer
1. **OBJ-platform-engineer-1** | Add `.github/workflows/validate.yml`: JSON-validate every `docs/data/**/*.geojson` (`jq`) and confirm `docs/index.html` + `docs/countries.config.js` (scoped to `COUNTRIES_ENABLED`) reference only files that exist (`python3` stdlib, `.github/scripts/check_refs.py`) — runs on push to `main`. **Resized M → S 2026-08-19** after a scoping report drafted and tested both the workflow YAML and the checker script against the real tree (clean pass) and a deliberately broken fixture (bad href, drifted `countries.config.js` filename, conflict markers in a `.geojson` — the exact historical incident class): all failed as expected, `exit 1`. Both files ship preinstalled on `ubuntu-latest` (`jq`, `python3`) — zero installs, no `package.json`, no build step, so it does not trip the structural veto (CI-only, produces no artifact, has zero deploy authority over `docs/`).
   unlocks:  a regulator or DC developer visiting the map right after a bad commit is not served a broken page — a check runs automatically instead of depending on a human remembering to run map-tester first — confirmed: no `.github/workflows/` directory exists anywhere in the repo
   evidence: workflow + script committed; a deliberately broken test commit (bad local href, a `countries.config.js` `file:` entry drifted from the real filename, or conflict markers in a `.geojson`) fails the Action; the real tree passes clean
   size:     S
   risk:     none to the deployed site (CI has no deploy authority); scoped to `COUNTRIES_ENABLED` to avoid false-failing on `egypt`/`senegal`/`namibia` placeholders that have no `docs/data/` directory yet. One open call for the ruling Chair/a veto-holding seat: the objective's original risk note said "pure shell" — the scoping report reads that as "no installs on the stock runner" rather than bash-only, since `jq`+`python3 stdlib` is materially more correct than a grep-only version (misses the `dataPath + file` runtime-concatenation pattern entirely). Flagged, not yet ruled on.
...
| 2026-08-19 | 23:15 | OBJ-map-debugger-1 | `2ab1212` | a regulator or DC developer who spots a data error can actually get the correction to land — "Report an error" no longer points to a non-deliverable `example.com` address |
