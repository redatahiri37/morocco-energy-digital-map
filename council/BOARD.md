...
2. **OBJ-frontend-engineer-3** | Give `.topbar` explicit overflow handling at ≤375px in `docs/style.css`
   ...
   size:     S
   risk:     low — layout-only change scoped to the existing 375px media query; `.topbar` is `display:flex` with `gap` and no `flex-wrap`/`overflow-x` today (confirmed by reading `docs/style.css:77-84,546-548`). Reconfirmed 2026-08-12: map-tester independently found a related `body`-level horizontal-overflow condition at 375px (`bodyScrollWidth:600` vs `clientWidth:375`), pre-existing, same root cause class.
3. **OBJ-frontend-engineer-4** | Clamp the hover-tooltip position to the viewport in `positionTooltip()` (`docs/app.js:906-911`)
   unlocks:  a DC developer or regulator hovering a substation, line, or plant near the left/right/top edge of the map canvas can actually read its capacity/voltage/source in the tooltip, instead of the card running off-screen — confirmed by reading source: `positionTooltip()` sets `tooltip.style.left/top` directly from `rect.left + point.x/point.y` with zero min/max clamping, and `.tooltip` (`docs/style.css:304-316`) is horizontally centered on that point via `transform:translate(-50%,-130%)` with `max-width:240px`, so any hover within ~120px of the map's left/right edge, or near its top edge, pushes the tooltip partially or fully outside the viewport
   evidence: hovering a feature within 120px of the map canvas's left, right, or top edge at a typical desktop width (≥1280px) still shows the full tooltip fully on-screen, not clipped or overflowing
   size:     S
   risk:     low — confined to `positionTooltip()`; must still track the cursor accurately for on-screen hovers, and must not regress the fixed `.popup` panel which is a separate element
   _filed 2026-08-12 sitting_
...
2. **OBJ-coord-validator-2** | Sample-verify `docs/data/morocco/national-hv.geojson` (947 ONEE 60 kV line features, `coord_method: osm_derived`) and `docs/data/morocco/transmission-lines.geojson` (541 WBG line features)
   ...
   **REPORT completed 2026-08-12** (full detail: `council/2026-08-12.md`): 0 FAIL / 1,066 PASS / 6 WARN (duplicate-vertex hygiene only) / 416 UNVERIFIED (411 ONEE-only 60 kV + 5 WBG-only planned; OSM/Nominatim/Overpass egress blocked from this environment, fallback external check could not run). Full-population cross-source matching found 536 `national-hv` and 536 `transmission-lines` features are the *same* real-world corridors double-cited (~300 m systematic reprojection offset) — true distinct-corridor count is ~952, not 1,488; this framing needs correcting wherever "1,488 features" is stated. Unblocks OBJ-map-debugger-2 on data-quality grounds: the 1,072 cross-corroborated 150/225/400 kV features are safe to wire in as a first slice; the 416 UNVERIFIED features should stay excluded or clearly marked until re-verified.
...
1. **OBJ-map-debugger-2** | Wire `docs/data/morocco/national-hv.geojson` (947 features) and `docs/data/morocco/transmission-lines.geojson` (541 features) into the live map as renderable layers
   ...
   size:     L
   risk:     performance (947+541 line features on one MapLibre source), visual clutter against the existing OIM grey grid layer. **Unblocked 2026-08-12**: OBJ-coord-validator-2's REPORT found 0 FAILs; needs splitting into a first S-sized slice (the 1,072 cross-corroborated 150/225/400 kV features only) before it is shippable — the 416 UNVERIFIED features should wait.
...
1. **OBJ-platform-engineer-1** | Add a minimal CI check on push to `main` (no build step, no bundler — pure validation)
   ...
   size:     S _(reclassified M→S 2026-08-12 — see REPORT)_
   ...
   **REPORT completed 2026-08-12** (full spec: `council/2026-08-12.md`): root-caused against the actual incident this objective exists to prevent (merged conflict markers broke `docs/app.js`'s syntax, masked by stale cache). Scoped a concrete 4-step `.github/workflows/docs-ci.yml`, `ubuntu-latest`, path-filtered to `docs/**`: (1) conflict-marker grep scoped to code/data extensions, (2) `node --check` every top-level `.js` file, (3) `jq empty` every `docs/data/**/*.geojson`, (4) referenced-file-existence check against `countries.config.js`/`index.html`. No `package.json`, no `npm install`, no `setup-node` — pure validation, does not trip the build-step veto. Ready to implement directly from spec.
...
| 2026-08-12 | 03:03 | OBJ-map-debugger-1 | `a61eb9d` | a regulator or DC developer who reports a wrong coordinate or stale figure via "Report an error" actually reaches a monitored inbox, instead of the report silently vanishing |
