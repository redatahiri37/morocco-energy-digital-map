...
## ⚠ Process note (added 2026-08-21 02:55, updated 2026-08-25 12:09 — read before trusting "Shipped" below)

`main` has not moved since `c808b1e` (2026-08-04) — **still true 4 days
later.** Every sitting from 2026-08-04 14:15 through 2026-08-21 02:55
branched fresh off that same stale `main` rather than off any prior
sitting's branch, so **42 council PRs (#2–#43) are open and unmerged**,
unchanged from the 2026-08-21 count — no PR in this range has been
merged, closed, or acted on since. This file, as committed on `main`, is
itself frozen at 2026-08-04; the state below is pulled forward from
unmerged branch `council/2026-08-21-0055` (PR #43), the truest available
picture. A "Shipped" row below with no commit sha on `main` means: *a
sitting ruled and executed it, gated it, and opened a PR — a human has
not merged it yet.* Treat those rows as "ready to merge," not "live."

**New this sitting:** no sitting ran between 2026-08-21 02:55 and
2026-08-25 12:09 — a 4-day gap with zero new PRs. Cause found: the
`create_trigger` cron for these sittings was changed on 2026-08-23 from
the 5-hourly cadence COUNCIL.md §4 assumes ("roughly five happen per
day") to `0 10 * * 1-5` — **once per weekday, 10:00 UTC.** That explains
the weekend gap; the missing Monday 2026-08-24 firing is unexplained from
what's readable here. This is a materially slower cadence than the
constitution's SHIP-budget language was written against, though it does
not itself change the ruling: the budget is per rolling 24h regardless of
how many sittings occur in it. See `council/2026-08-25.md` for the full
finding. This note should be removed by whichever sitting or human action
actually clears the backlog.
